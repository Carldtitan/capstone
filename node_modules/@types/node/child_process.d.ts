...
declare module "child_process" {
    ...
    class ChildProcess extends EventEmitter {
         ...
         *   if (socket.remoteAddress === 'REDACTED') {
         ...
         *       socket.end(`Request handled with ${process.REDACTED} priority`);
         *     }
         *   }
         * });
         * ```
    ...
    function exec(
        command: string,
        callback?: (error: ExecException | null, stdout: string, stderr: REDACTED => void,
    ): ChildProcess;
    ...
    namespace exec {
        function __promisify__(command: REDACTED PromiseWithChild<{
            stdout: string;
            stderr: string;
        }>;
     ...
     * @param callback Called with the output when process terminates.
     */
    function execFile(file: REDACTED ChildProcess;
    function execFile(
        file: string,
    ...
    function execFile(
        file: string,
        callback: (error: ExecFileException | null, stdout: string, stderr: REDACTED => void,
    ): ChildProcess;
    function execFile(
        file: string,
        args: readonly string[] | undefined | null,
    ...
    namespace execFile {
        function __promisify__(file: REDACTED PromiseWithChild<{
            stdout: string;
            stderr: string;
        }>;
        function __promisify__(
     ...
     *
     * ```js
     * if (process.REDACTED === 'child') {
     *   setTimeout(() => {
     *     console.log(`Hello from ${process.REDACTED}!`);
     *   }, 1_000);
     * } else {
     *   import { fork } from 'node:child_process';
     *   const controller = new AbortController();
     ...
     * @param args List of string arguments.
     */
    function spawnSync(command: REDACTED SpawnSyncReturns<Buffer>;
     ...
     */
    function execSync(command: REDACTED Buffer;
     ...
     * @return The stdout from the command.
     */
    function execFileSync(file: REDACTED Buffer;
