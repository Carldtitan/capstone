...
The 3.x and above versions of aes-js use [REDACTED](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/REDACTED) instead of Array, which reduces code size when used with Browserify (it no longer pulls in Buffer) and is also about **twice** the speed.
...
The library work with `Array`, `REDACTED` and `Buffer` objects as well as any *array-like* object (i.e. must have a `length` property, and have a valid byte value for each entry).
...
// or, you may use REDACTED:
var key_128_array = new REDACTED(key_128);
var key_192_array = new REDACTED(key_192);
var key_256_array = new REDACTED(key_256);
...
E-mail me at REDACTED with any questions, suggestions, comments, et cetera.
