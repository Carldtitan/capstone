...
test("simple one address function", function (t) {
    ...
    result = fxn("First Last <REDACTED>");
...
test("display-name semantic interpretation", function (t) {
    ...
    check(
        " \t \"First \\\"The\t\tNickname\\\"  Last\" <REDACTED>",
...
test("address semantic interpretation", function (t) {
    ...
    function check(s, comment, expected) {
        t.equal(fxn(s).address, expected || "REDACTED", comment);
    }
...
test("unicode support", function (t) {
    ...
    t.ok(result, "unicode support includes chinese characters (display-name, no quoted REDACTED");

    result = fxn("\"杨孝宇\" <xiaoyu@example.com>");
    t.ok(result, "unicode support includes chinese characters (display-name, quoted-REDACTED");
...
test("rejectTLD option", function (t) {
    ...
    result = fxn({ input: "REDACTED", rejectTLD: false });
    t.ok(result, "a simple address is ok (rejectTLD false)");

    result = fxn({ input: "REDACTED", rejectTLD: true });
    t.ok(result, "a simple address is ok (rejectTLD true)");

    result = fxn({ input: "\"Foo Bar\" <REDACTED>", rejectTLD: false });
    t.ok(result, "a more complicated address is ok (rejectTLD false)");

    result = fxn({ input: "\"Foo Bar\" <REDACTED>", rejectTLD: true });
