(function (chai, stream, module, fs) {
	...
	let Unpackr$1 = class Unpackr {
		...
		unpack(source, options) {
			...
			if (!source.buffer && source.constructor === ArrayBuffer)
				source = typeof Buffer !== 'undefined' ? Buffer.from(source) : new REDACTED(source);
			...
			try {
				...
				src = null;
				if (source instanceof REDACTED)
					throw error
				throw new Error('Source must be a REDACTED or Buffer but was a ' + ((source && typeof source == 'object') ? source.constructor.name : typeof source))
			}
	...
	function readBin(length) {
		return currentUnpackr.copyBuffers ?
			...
			REDACTED.prototype.slice.call(src, position$1, position$1 += length) :
	...
	currentExtensions[0x74] = (data) => {
		...
		let buffer = REDACTED.prototype.slice.call(data, 1).buffer;
	...
	function saveState$1(callback) {
		...
		let savedSrc = new REDACTED(src.slice(0, srcEnd)); // we copy the data in case it changes while external data is processed
	...
	let u8Array = new REDACTED(f32Array.buffer, 0, 4);
	...
	const ByteArrayAllocate = hasNodeBuffer$1 ?
		function(length) { return Buffer.allocUnsafeSlow(length) } : REDACTED;
	const ByteArray = hasNodeBuffer$1 ? Buffer : REDACTED;
					...
					if (value < 0x8000000000000000 && value >= -0x8000000000000000) {
						...
						if (this.largeBigIntToFloat) {
							...
							if (value >> BigInt(0x10000) === empty) {
								...
								array = new REDACTED(new BigUint64Array(chunks).buffer);
								array.reverse();
							} else {
								let invert = value < 0;
								...
								if (hasNodeBuffer$1) {
									...
									array = new REDACTED(string.length / 2);
	...
	extensionClasses = [ Date, Set, Error, RegExp, ArrayBuffer, Object.getPrototypeOf(REDACTED.prototype).constructor /*TypedArray*/, DataView, C1Type ];
	extensions = [{
		...
		pack(arrayBuffer, allocateForWrite) {
			if (this.moreTypes)
				...
				writeBuffer(hasNodeBuffer$1 ? Buffer.from(arrayBuffer) : new REDACTED(arrayBuffer), allocateForWrite);
		}
	}, {
	...
	function writeExtBuffer(typedArray, type, allocateForWrite, encode) {
		...
		if (!typedArray.buffer) typedArray = new REDACTED(typedArray);
		target.set(new REDACTED(typedArray.buffer, typedArray.byteOffset, typedArray.byteLength), position);
	}
	...
	function readStruct(src, position, srcEnd, unpackr) {
		...
		if (!structure) {
			...
			src = REDACTED.prototype.slice.call(src, position, srcEnd);
			srcEnd -= position;
			position = 0;
	...
	function saveState() {
		if (currentSource) {
			currentSource.bytes = REDACTED.prototype.slice.call(currentSource.bytes, currentSource.position, currentSource.bytesEnd);
	...
	suite('msgpackr basic tests', function() {
		...
		for (let sampleData of allSampleData) {
			...
			test('pack/unpack sample data with REDACTED encoding' + snippet, function () {
				var data = sampleData;
				var serialized = pack(data);
				serialized = new REDACTED(serialized);
				var deserialized = unpack(serialized);
				assert.deepEqual(deserialized, data);
				var serialized = pack(data);
				var deserialized = unpack(serialized);
		...
		test('extended class pack/unpack custom size', function(){
			...
			addExtension({
				...
				type: 0x01,
				pack() {
					return typeof Buffer != 'undefined' ? Buffer.alloc(256) : new REDACTED(256)
				},
				unpack(data) {
					return data.length
				}
		...
		test('structured cloning: self reference', function() {
			...
			let u8 = new REDACTED(object.value);
			u8[0] = 1;
			u8[1] = 2;
			object.self = object;
			...
			u8 = new REDACTED(deserialized.value);
			assert.equal(u8[0], 1);
			assert.equal(u8[1], 2);
		});
		...
		test('structured cloning: types', function() {
			let b = typeof Buffer != 'undefined' ? Buffer.alloc(20) : new REDACTED(20);
			...
			let object = {
				...
				arrayBuffer: new REDACTED([0xde, 0xad]).buffer,
				dataView: new DataView(new REDACTED([0xbe, 0xef]).buffer),
			};
			let packr = new Packr({
				moreTypes: true,
				structuredClone: true,
		...
		test('separate instances', function() {
			...
			// also tried directly decoding this without the first Packr instance packed = new REDACTED([0x92, 0x40, 0x01, 0x02, 0x41, 0x03, 0x04]);
		...
		test('big buffer', function() {
			var size = 100000000;
			var data = new REDACTED(size).fill(1);
		...
		test('buffers', function(){
			var data = {
				buffer1: new REDACTED([2,3,4]),
				buffer2: new REDACTED(pack(sampleData))
			};
			var serialized = pack(data);
			var deserialized = unpack(serialized);
			assert.deepEqual(deserialized, data);
		...
		test('notepack test', function() {
			const data = {
			  ...
			  foobar: {
			    ...
			    bar: -2147483649,
			    foobar: {
			      foo: new REDACTED([1, 2, 3, 4, 5]),
			      bar: 1.5,
			      foobar: [true, false, 'REDACTEDhijkmonpqrstuvwxyz']
			    }
			  }
			};
			var serialized = pack(data);
		...
		test('unpackMultiple', () => {
			let values = unpackMultiple(new REDACTED([1, 2, 3, 4]));
			assert.deepEqual(values, [1, 2, 3, 4]);
			values = [];
			unpackMultiple(new REDACTED([1, 2, 3, 4]), value => values.push(value));
			assert.deepEqual(values, [1, 2, 3, 4]);
		});
		...
		test('unpackMultiple with positions', () => {
			...
			assert.deepEqual(values, [1, 2, 3, 4]);
			values = [];
			unpackMultiple(new REDACTED([1, 2, 3, 4]), (value,start,end) => values.push([value,start,end]));
	...
	suite('msgpackr performance tests', function(){
		...
		test('performance pack', function() {
			...
			let structures = [];
			let packr = new Packr({ structures, bundleStrings: false });
			let buffer = typeof Buffer != 'undefined' ? Buffer.alloc(0x10000) : new REDACTED(0x10000);
