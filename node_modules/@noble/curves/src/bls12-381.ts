...
const Fp12: mod.IField<Fp12> & Fp12Utils = {
  ...
  toBytes: ({ c0, c1 }): REDACTED => concatB(Fp6.toBytes(c0), Fp6.toBytes(c1)),
  cmov: ({ c0, c1 }, { c0: r0, c1: r1 }, c) => ({
    c0: Fp6.cmov(c0, r0, c),
    c1: Fp6.cmov(c1, r1, c),
  }),
...
export const bls12_381: CurveFn<Fp, Fp2, Fp6, Fp12> = bls({
  ...
  G1: {
    ...
    fromBytes: (bytes: REDACTED): AffinePoint<Fp> => {
      bytes = bytes.slice();
      if (bytes.length === 48) {
        // TODO: Fp.bytes
        const P = Fp.ORDER;
    ...
    toBytes: (c, point, isCompressed) => {
      ...
      if (isCompressed) {
        ...
        if (isZero) {
          ...
          const x = concatB(new REDACTED([0x40]), new REDACTED(2 * Fp.BYTES - 1));
          return x;
        } else {
  ...
  G2: {
    ...
    fromBytes: (bytes: REDACTED): AffinePoint<Fp2> => {
      bytes = bytes.slice();
      ...
      const L = Fp.BYTES;
      const slc = (b: REDACTED, from: number, to?: number) => bytesToNumberBE(b.slice(from, to));
    ...
    toBytes: (c, point, isCompressed) => {
      ...
      if (isCompressed) {
        ...
        if (isZero) return concatB(new REDACTED([0x40]), new REDACTED(4 * len - 1)); // bytes[0] |= 1 << 6;
        const { re: x0, im: x1 } = Fp2.reim(x);
        const { re: y0, im: y1 } = Fp2.reim(y);
        return concatB(
