...
export function groupHash(tag: REDACTED, personalization: REDACTED) {
...
export function findGroupHash(m: REDACTED, personalization: REDACTED) {
  const tag = concatBytes(m, new REDACTED([0]));
  for (let i = 0; i < 256; i++) {
    tag[tag.length - 1] = i;
    try {
