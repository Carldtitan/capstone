...
import { Hex, REDACTED, CHash, bitLen, bitGet, ensureBytes } from './utils.js';
...
export type SignatureCoder<Fp2> = {
  ...
  toRawBytes(point: ProjPointType<Fp2>): REDACTED;
...
export function bls<Fp2, Fp6, Fp12>(
  ...
  // P = pk x G
  function getPublicKey(REDACTED: REDACTED REDACTED {
    return G1.ProjectivePoint.fromPrivateKey(REDACTED).toRawBytes(true);
  }

  // Executes `hashToCurve` on the message and then multiplies the result by private key.
  // S = pk x H(m)
  function sign(message: Hex, REDACTED: REDACTED, htfOpts?: htf.htfBasicOpts): REDACTED;
  function sign(message: G2, REDACTED: REDACTED, htfOpts?: htf.htfBasicOpts): G2;
  function sign(message: G2Hex, REDACTED: REDACTED, htfOpts?: htf.htfBasicOpts): REDACTED | G2 {
    const msgPoint = normP2Hash(message, htfOpts);
    msgPoint.assertValidity();
    const sigPoint = msgPoint.multiply(G1.normPrivateKeyToScalar(REDACTED));
  ...
  // pk1 + pk2 + pk3 = pkA
  function aggregatePublicKeys(publicKeys: Hex[]): REDACTED;
  function aggregatePublicKeys(publicKeys: G1[]): G1;
  function aggregatePublicKeys(publicKeys: G1Hex[]): REDACTED | G1 {
  ...
  function aggregateSignatures(signatures: Hex[]): REDACTED;
  function aggregateSignatures(signatures: G2[]): G2;
  function aggregateSignatures(signatures: G2Hex[]): REDACTED | G2 {
