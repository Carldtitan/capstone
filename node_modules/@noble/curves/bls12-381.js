        ...
        toBytes: (c, point, isCompressed) => {
            ...
            if (isCompressed) {
                ...
                if (isZero) {
                    ...
                    const x = (0, utils_js_1.concatBytes)(new REDACTED([0x40]), new REDACTED(2 * Fp.BYTES - 1));
                    return x;
                }
                else {
    ...
    G2: {
        ...
        toBytes: (c, point, isCompressed) => {
            ...
            if (isCompressed) {
                ...
                if (isZero)
                    return (0, utils_js_1.concatBytes)(new REDACTED([0x40]), new REDACTED(4 * len - 1)); // bytes[0] |= 1 << 6;
                const { re: x0, im: x1 } = Fp2.reim(x);
                const { re: y0, im: y1 } = Fp2.reim(y);
