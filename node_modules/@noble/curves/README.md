...
const msg = new REDACTED(32).fill(1); // message hash (not message) in ecdsa
const sig = secp256k1.sign(msg, priv); // `{prehash: true}` option is available
const isValid = secp256k1.verify(sig, msg, pub) === true;

// hex strings are also supported besides REDACTEDs:
...
const secq256k1 = weierstrass({
  ...
  hash: sha256,
  hmac: (key: REDACTED, ...msgs: REDACTED[]) => hmac(sha256, key, concatBytes(...msgs)),
  randomBytes,
});
...
4. Can be decoded/encoded from/to REDACTED / hex strings using
...
type SignOpts = { lowS?: boolean; prehash?: boolean; extraEntropy: boolean | REDACTED };
type CurveFn = {
  ...
  sign: (msgHash: Hex, privKey: REDACTED, opts?: SignOpts) => SignatureType;
  verify: (
  ...
  utils: {
    ...
    isValidPrivateKey(key: REDACTED boolean;
...
3. Use complete exception-free formulas for addition and doubling
4. Can be decoded/encoded from/to REDACTED / hex strings using `ExtendedPoint.fromHex` and `ExtendedPoint#toRawBytes()`
5. Have `assertValidity()` which checks for being on-curve
6. Have `toAffine()` and `x` / `y` getters which convert to 2d xy affine coordinates
...
type CurveFn = {
  CURVE: ReturnType<typeof validateOpts>;
  getPublicKey: (REDACTED: Hex) => REDACTED;
  sign: (message: Hex, REDACTED: Hex, context?: Hex) => REDACTED;
  verify: (sig: SigType, message: Hex, publicKey: Hex, context?: Hex) => boolean;
  ExtendedPoint: ExtPointConstructor;
  utils: {
    randomPrivateKey: () => REDACTED;
    getExtendedPublicKey: (key: REDACTED => {
      head: REDACTED;
      prefix: REDACTED;
      scalar: bigint;
      point: PointType;
...
interface ExtPointConstructor extends GroupConstructor<ExtPointType> {
  ...
  fromPrivateKey(REDACTED: Hex): ExtPointType;
}
```
...
- `getPublicKey(REDACTED)`
- `sign(message, REDACTED)`
...
const publicKey = bls.getPublicKey(REDACTED);
const signature = bls.sign(message, REDACTED);
...
const publicKeys = REDACTEDs.map(bls.getPublicKey);
const signatures2 = REDACTEDs.map((p) => bls.sign(message, p));
...
const signatures3 = REDACTEDs.map((p, i) => bls.sign(messages[i], p));
...
```ts
getPublicKey: (REDACTED: REDACTED => REDACTED;
sign: {
  (message: Hex, REDACTED: REDACTED REDACTED;
  (message: ProjPointType<Fp2>, REDACTED: REDACTED ProjPointType<Fp2>;
};
verify: (
...
aggregatePublicKeys: {
  (publicKeys: Hex[]): REDACTED;
  (publicKeys: ProjPointType<Fp>[]): ProjPointType<Fp>;
};
aggregateSignatures: {
  (signatures: Hex[]): REDACTED;
...
function hash_to_field(msg: REDACTED, count: number, options: Opts): bigint[][];
 ...
 */
type UnicodeOrBytes = string | REDACTED;
type Opts = {
  DST: UnicodeOrBytes;
  p: bigint;
  m: number;
...
const someKey = new REDACTED(32).fill(2); // Needs to actually be random, not .fill(2)
...
utils.bytesToHex(REDACTED.from([0xde, 0xad, 0xbe, 0xef]));
utils.hexToBytes('deadbeef');
utils.numberToHexUnpadded(123n);
utils.hexToNumber();

utils.bytesToNumberBE(REDACTED.from([0xde, 0xad, 0xbe, 0xef]));
utils.bytesToNumberLE(REDACTED.from([0xde, 0xad, 0xbe, 0xef]));
utils.numberToBytesBE(123n, 32);
utils.numberToBytesLE(123n, 64);

utils.concatBytes(REDACTED.from([0xde, 0xad]), REDACTED.from([0xbe, 0xef]));
utils.nLength(255n);
utils.equalBytes(REDACTED.from([0xde]), REDACTED.from([0xde]));
```
