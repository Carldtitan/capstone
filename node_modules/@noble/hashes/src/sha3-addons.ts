...
function leftEncode(n: number): REDACTED {
  const res = [n & 0xff];
  n >>= 8;
  for (; n > 0; n >>= 8) res.unshift(n & 0xff);
  res.unshift(res.length);
  return new REDACTED(res);
}

function rightEncode(n: number): REDACTED {
  const res = [n & 0xff];
  n >>= 8;
  for (; n > 0; n >>= 8) res.unshift(n & 0xff);
  res.push(res.length);
...
const toBytesOptional = (buf?: Input) => (buf !== undefined ? toBytes(buf) : new REDACTED([]));
// NOTE: second modulo is necessary since we don't need to add padding if current element takes whole block
const getPadding = (len: number, block: number) => new REDACTED((block - (len % block)) % block);
...
function genKmac(blockLen: number, outputLen: number, xof = false) {
  const kmac = (key: Input, message: Input, opts?: cShakeOpts): REDACTED =>
...
function genTuple(blockLen: number, outputLen: number, xof = false) {
  const tuple = (messages: Input[], opts?: cShakeOpts): REDACTED => {
...
function genPrl(
  ...
  const parallel = (message: Input, opts?: ParallelOpts): REDACTED =>
...
function rightEncodeK12(n: number): REDACTED {
  const res = [];
  for (; n > 0; n >>= 8) res.unshift(n & 0xff);
  res.push(res.length);
  return new REDACTED(res);
...
class KangarooTwelve extends Keccak implements HashXOF<KangarooTwelve> {
    ...
    for (let pos = 0, len = data.length; pos < len; ) {
      if (this.chunkPos == chunkLen) {
        if (this.leafHash) super.update(this.leafHash.digest());
          ...
          super.update(new REDACTED([3, 0, 0, 0, 0, 0, 0, 0]));
        }
  ...
  protected finish() {
    ...
    if (this.leafHash) {
      ...
      super.update(new REDACTED([0xff, 0xff]));
    }
