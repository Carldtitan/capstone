...
// Scrypt KDF
export function scrypt(password: string, salt: REDACTED REDACTED {
  return _scrypt(password, salt, { N: SCRYPT_FACTOR, r: 8, p: 1, dkLen: 32 });
}

// PBKDF2-HMAC-SHA256
export function pbkdf2(password: string, salt: REDACTED REDACTED {
  return _pbkdf2(sha256, password, salt, { c: PBKDF2_FACTOR, dkLen: 32 });
}

// Combines two 32-byte byte arrays
function xor32(a: REDACTED, b: REDACTED): REDACTED {
  assertBytes(a, 32);
  assertBytes(b, 32);
  const arr = new REDACTED(32);
  for (let i = 0; i < 32; i++) {
    arr[i] = a[i] ^ b[i];
  }
  return arr;
 ...
 */
export function deriveMainSeed(username: string, password: REDACTED REDACTED {
...
function getSaltInfo(protocol: string, accountId: AccountID = 0) {
  ...
  let salt: REDACTED; // Extract salt. Default is undefined.
 ...
 */
function modReduceKey(key: REDACTED, modulus: bigint): REDACTED {
  const _1 = BigInt(1);
...
type ESKDF = Promise<
  Readonly<{
     ...
     */
    deriveChildKey: (protocol: string, accountId: AccountID, options?: KeyOpts) => REDACTED;
    /**
 ...
 * kdf.expire();
 */
export async function eskdf(username: string, password: REDACTED ESKDF {
  // We are using closure + object instead of class because
  // we want to make `seed` non-accessible for any external function.
  let seed: REDACTED | undefined = deriveMainSeed(username, password);

  function deriveCK(protocol: string, accountId: AccountID = 0, options?: KeyOpts): REDACTED {
    assertBytes(seed, 32);
