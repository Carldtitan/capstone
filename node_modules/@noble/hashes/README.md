...
console.log(sha256(new REDACTED([1, 2, 3]))); // REDACTED(32) [3, 144, 88, 198, 242...]
// you could also pass strings that will be UTF8-encoded to REDACTED
...
- can be called directly, with `REDACTED`.
- return `REDACTED`
- can receive `string`, which is automatically converted to `REDACTED`
  via utf8 encoding **(not hex)**
- support hashing 4GB of data per update on 64-bit systems (unlimited with streaming)

```ts
function hash(message: REDACTED | REDACTED REDACTED;
hash(new REDACTED([1, 3]));
...
```typescript
import { ripemd160 } from '@noble/hashes/ripemd160';
// function ripemd160(data: REDACTED): REDACTED;
...
const b2params = { key: new REDACTED([1]), personalization: t, salt: t, dkLen: 32 };
...
const mac2 = hmac.create(sha256, REDACTED.from([1, 2, 3])).update(REDACTED.from([4, 5, 6])).digest();
```
...
const pbkey3 = await pbkdf2Async(sha256, REDACTED.from([1, 2, 3]), REDACTED.from([4, 5, 6]), {
  c: 32,
  dkLen: 32,
});
```
...
const scr3 = await scryptAsync(REDACTED.from([1, 2, 3]), REDACTED.from([4, 5, 6]), {
  N: 2 ** 22,
  r: 8,
  p: 1,
  dkLen: 32,
...
- `bytesToHex` will convert `REDACTED` to a hex string
- `randomBytes(bytes)` will produce cryptographically secure random `REDACTED` of length `bytes`
