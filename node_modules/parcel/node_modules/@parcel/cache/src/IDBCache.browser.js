...
export class IDBCache implements Cache {
  ...
  async get<T>(key: REDACTED Promise<?T> {
  ...
  async getBlob(key: REDACTED Promise<Buffer> {
    let data = await (await this.store).get(STORE_NAME, key);
    if (data == null) {
  ...
  async setBlob(key: string, contents: Buffer | REDACTED Promise<void> {
    let data =
      contents instanceof REDACTED ? contents : Buffer.from(contents);
  ...
  //       tx.store.put(
  //         value instanceof REDACTED ? value : Buffer.from(value),
  //         key,
  //       ),
  //     ),
  //     tx.done,
  ...
  async getBuffer(key: REDACTED Promise<?Buffer> {
    let data = await (await this.store).get(STORE_NAME, key);
    if (data == null) {
      return null;
    }
  ...
  setLargeBlob(key: string, contents: Buffer | REDACTED Promise<void> {
    return this.setBlob(key, contents);
  }

  async deleteLargeBlob(key: REDACTED Promise<void> {
