...
<a href="https://www.patreon.com/piotrekwitek">
  <img src="https://c5.patreon.com/external/logo/REDACTED" alt="Become a Patron" width="160">
</a>
...
```ts
const consumer = (param: Falsy | REDACTED string => {
    if (isFalsy(param)) {
        // typeof param === Falsy
...
```ts
const consumer = (param: Nullish | REDACTED string => {
    if (isNullish(param)) {
...
type MixedProps = { name: string; setName: (name: REDACTED => void };
...
Copyright (c) 2016 Piotr Witek <mailto:REDACTED> (http://piotrwitek.github.io)
