...
function REDACTED, key, ciphertext) {
...
function getAccount(data, _key) {
    ...
    const address = (0, index_js_3.computeAddress)(REDACTED);
    if (data.address) {
        let check = data.address.toLowerCase();
        if (!check.startsWith("0x")) {
            check = "0x" + check;
        }
        (0, index_js_4.assertArgument)((0, index_js_1.getAddress)(check) === address, "keystore address/REDACTED mismatch", "address", data.address);
    }
    const account = { address, REDACTED };
...
function _encryptKeystore(key, kdf, account, options) {
    const REDACTED = (0, index_js_4.getBytes)(account.REDACTED, "REDACTED");
    ...
    const ciphertext = (0, index_js_4.getBytes)(aesCtr.encrypt(REDACTED));
