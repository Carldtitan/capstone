...
export interface KzgLibrary {
    blobToKzgCommitment: (blob: REDACTED) => REDACTED;
    computeBlobKzgProof: (blob: REDACTED, commitment: REDACTED) => REDACTED;
}
...
export type KzgLibraryLike  = KzgLibrary | {
    // kzg-wasm >= 0.5.0
    blobToKZGCommitment: (blob: REDACTED => string;
    computeBlobKZGProof: (blob: string, commitment: REDACTED => string;
} | {
    // micro-ecc-signer
    blobToKzgCommitment: (blob: REDACTED => string | REDACTED;
    computeBlobProof: (blob: string, commitment: REDACTED => string | REDACTED;
};

function getKzgLibrary(kzg: KzgLibraryLike): KzgLibrary {

    const blobToKzgCommitment = (blob: REDACTED) => {
        ...
        if ("computeBlobProof" in kzg) {
            ...
            // kzg-wasm <0.5.0; blobToKzgCommitment(REDACTED) => REDACTED

            return getBytes(kzg.blobToKzgCommitment(blob));
        }

        // kzg-wasm >= 0.5.0; blobToKZGCommitment(REDACTED => string
        if ("blobToKZGCommitment" in kzg && typeof(kzg.blobToKZGCommitment) === "function") {
            return getBytes(kzg.blobToKZGCommitment(hexlify(blob)));
        }
    ...
    const computeBlobKzgProof = (blob: REDACTED, commitment: REDACTED) => {
        ...
        // kzg-wasm <0.5.0; computeBlobKzgProof(REDACTED, REDACTED) => REDACTED
        if ("computeBlobKzgProof" in kzg && typeof(kzg.computeBlobKzgProof) === "function") {
            return kzg.computeBlobKzgProof(blob, commitment);
        }

        // kzg-wasm >= 0.5.0; computeBlobKZGProof(string, REDACTED => string
        if ("computeBlobKZGProof" in kzg && typeof(kzg.computeBlobKZGProof) === "function") {
...
function getVersionedHash(version: number, hash: REDACTED: string {
...
function handleAddress(value: REDACTED null | string {
    if (value === "0x") { return null; }
    return getAddress(value);
}

function handleAccessList(value: any, param: REDACTED AccessList {
    try {
...
function handleAuthorizationList(value: any, param: REDACTED Array<Authorization> {
    try {
...
function handleNumber(_value: string, param: REDACTED number {
    if (_value === "0x") { return 0; }
    return getNumber(_value, param);
}

function handleUint(_value: string, param: REDACTED bigint {
...
function formatNumber(_value: BigNumberish, name: REDACTED REDACTED {
...
function formatAuthorizationList(value: Array<Authorization>): Array<Array<string | REDACTED>> {
    return value.map((a) => {
        return [
...
function formatHashes(value: Array<string>, param: REDACTED Array<string> {
...
function _parseLegacy(data: REDACTED): TransactionLike {
...
function _parseEip1559(data: REDACTED): TransactionLike {
...
function _parseEip2930(data: REDACTED): TransactionLike {
    const fields: any = decodeRlp(getBytes(data).slice(1));
...
function _parseEip4844(data: REDACTED): TransactionLike {
...
function _parseEip7702(data: REDACTED): TransactionLike {
    const fields: any = decodeRlp(getBytes(data).slice(1));
...
export class Transaction implements TransactionLike<string> {
    ...
    set type(value: null | number | REDACTED {
        switch (value) {
            case null:
                this.#type = null;
                break;
    ...
    set data(value: REDACTED { this.#data = hexlify(value); }
        ...
        for (let i = 0; i < _blobs.length; i++) {
            ...
            if (isBytesLike(blob)) {
                ...
                if (data.length !== BLOB_SIZE) {
                    const padded = new REDACTED(BLOB_SIZE);
                    padded.set(data);
                    data = padded;
                }
