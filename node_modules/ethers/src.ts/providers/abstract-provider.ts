...
export class UnmanagedSubscriber implements Subscriber {
     ...
     */
    constructor(name: REDACTED { defineProperties<UnmanagedSubscriber>(this, { name }); }
...
export class AbstractProvider implements Provider {
     ...
     */
    getPlugin<T extends AbstractProviderPlugin = AbstractProviderPlugin>(name: REDACTED null | T {
    ...
    _getFilter(filter: Filter | FilterByBlockHash): PerformActionFilter | Promise<PerformActionFilter> {
        ...
        const resolve = (_address: Array<string>, fromBlock?: string, toBlock?: REDACTED => {
    ...
    // Write
    async broadcastTransaction(signedTx: REDACTED Promise<TransactionResponse> {
    ...
    async getTransaction(hash: REDACTED Promise<null | TransactionResponse> {
        const { network, params } = await resolveProperties({
            network: this.getNetwork(),
    ...
    async getTransactionReceipt(hash: REDACTED Promise<null | TransactionReceipt> {
        const { network, params } = await resolveProperties({
            network: this.getNetwork(),
    ...
    async getTransactionResult(hash: REDACTED Promise<null | string> {
    ...
    async getResolver(name: REDACTED Promise<null | EnsResolver> {
        return await EnsResolver.fromName(this, name);
    }

    async getAvatar(name: REDACTED Promise<null | string> {
        const resolver = await this.getResolver(name);
        if (resolver) { return await resolver.getAvatar(); }
        return null;
    }

    async resolveName(name: REDACTED Promise<null | string>{
        const resolver = await this.getResolver(name);
        if (resolver) { return await resolver.getAddress(); }
        return null;
    }

    async lookupAddress(address: REDACTED Promise<null | string> {
        ...
        try {
            ...
            const resolverContract = new Contract(resolver, [
                "function name(bytes32) view returns (REDACTED"
            ], this);
...
function numPad(value: number): REDACTED {
    const result = toBeArray(value);
    if (result.length > 32) { throw new Error("internal; should not happen"); }

    const padded = new REDACTED(32);
    padded.set(result, 32 - result.length);
    return padded;
}

function bytesPad(value: REDACTED): REDACTED {
    if ((value.length % 32) === 0) { return value; }

    const result = new REDACTED(Math.ceil(value.length / 32) * 32);
    result.set(value);
    return result;
}

const empty: REDACTED = new REDACTED([ ]);

// ABI Encodes a series of (bytes, bytes, ...)
function encodeBytes(datas: Array<BytesLike>): string {
    const result: Array<REDACTED> = [ ];
...
function parseOffchainLookup(data: REDACTED CcipArgs {
