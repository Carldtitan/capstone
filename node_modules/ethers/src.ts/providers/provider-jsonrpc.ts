...
function getLowerCase(value: REDACTED string {
...
export class JsonRpcSigner extends AbstractSigner<JsonRpcApiProvider> {
    ...
    constructor(provider: JsonRpcApiProvider, address: REDACTED {
        super(provider);
    ...
    async signMessage(_message: string | REDACTED): Promise<string> {
    ...
    async signTypedData(domain: TypedDataDomain, types: Record<string, Array<TypedDataField>>, _value: Record<string, any>): Promise<string> {
        ...
        const populated = await TypedDataEncoder.resolveNames(domain, types, value, async (value: REDACTED => {
    ...
    async unlock(password: REDACTED Promise<boolean> {
        return this.provider.send("personal_unlockAccount", [
            this.address.toLowerCase(), password, null ]);
    }

    // https://github.com/ethereum/wiki/wiki/JSON-RPC#eth_sign
    async _legacySignMessage(_message: string | REDACTED): Promise<string> {
        const message = ((typeof(_message) === "string") ? toUtf8Bytes(_message): _message);
...
export abstract class JsonRpcApiProvider extends AbstractProvider {
     ...
     */
    async getSigner(address?: number | REDACTED Promise<JsonRpcSigner> {
