...
function getApiKey(name: REDACTED string {
    switch (name) {
...
export class ChainstackProvider extends JsonRpcProvider implements CommunityResourcable {
     ...
     */
    constructor(_network?: Networkish, apiKey?: null | REDACTED {
        if (_network == null) { _network = "mainnet"; }
        const network = Network.from(_network);

        if (apiKey == null) { apiKey = REDACTED; }
    ...
    _getProvider(chainId: number): AbstractProvider {
        try {
            return new ChainstackProvider(chainId, REDACTED);
        } catch (error) { }
        return super._getProvider(chainId);
    }

    isCommunityResource(): boolean {
        return (REDACTED === getApiKey(this._network.name));
    }
     ...
     */
    static getRequest(network: Network, apiKey?: null | REDACTED FetchRequest {
        if (apiKey == null) { apiKey = REDACTED; }
