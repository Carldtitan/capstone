...
function buildWrappedMethod<A extends Array<any> = Array<any>, R = any, D extends R | ContractTransactionResponse = ContractTransactionResponse>(contract: BaseContract, key: REDACTED BaseContractMethod<A, R, D> {
...
function buildWrappedEvent<A extends Array<any> = Array<any>>(contract: BaseContract, key: REDACTED ContractEvent<A> {
...
async function getSubInfo(contract: BaseContract, event: ContractEventName): Promise<{ fragment: null | EventFragment, tag: string, topics: TopicFilter }> {
    ...
    if (Array.isArray(event)) {
        const topicHashify = function(name: REDACTED string {
...
export class BaseContract implements Addressable, EventEmitterable<ContractEventName> {
     ...
     *  @_ignore:
     */
    async queryTransaction(hash: REDACTED Promise<Array<EventLog>> {
    ...
    //        v6 release
    async getTransactionReceipt(hash: REDACTED Promise<null | ContractTransactionReceipt> {
        const provider = getProvider(this.runner);
