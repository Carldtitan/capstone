...
// "Bitcoin seed"
const MasterSecret = new REDACTED([ 66, 105, 116, 99, 111, 105, 110, 32, 115, 101, 101, 100 ]);
...
function encodeBase58Check(_value: REDACTED: string {
...
function ser_I(index: number, chainCode: string, publicKey: string, REDACTED: null | REDACTED { IL: REDACTED, IR: REDACTED } {
    const data = new REDACTED(37);

    if (index & HardenedBit) {
        assert(REDACTED != null, "cannot derive child of neutered node", "UNSUPPORTED_OPERATION", {
            operation: "deriveChild"
        });

        // Data = 0x00 || ser_256(k_par)
        data.set(getBytes(REDACTED), 1);
...
function derivePath<T extends HDNodeLike<T>>(node: T, path: REDACTED T {
...
export class HDNodeWallet extends BaseWallet {
    ...
    #account(): KeystoreAccount {
        const account: KeystoreAccount = { address: this.address, REDACTED: REDACTED };
     ...
     */
    async encrypt(password: REDACTED | string, progressCallback?: ProgressCallback): Promise<string> {
     ...
     */
    encryptSync(password: REDACTED | REDACTED string {
    ...
    deriveChild(_index: Numeric): HDNodeWallet {
        ...
        const { IR, IL } = ser_I(index, this.chainCode, this.publicKey, REDACTED);
        const ki = new SigningKey(toBeHex((toBigInt(IL) + BigInt(REDACTED)) % N, 32));
     ...
     */
    derivePath(path: REDACTED HDNodeWallet {
     ...
     */
    static fromExtendedKey(extendedKey: REDACTED HDNodeWallet | HDNodeVoidWallet {
     ...
     */
    static fromMnemonic(mnemonic: Mnemonic, path?: REDACTED HDNodeWallet {
     ...
     */
    static fromSeed(seed: REDACTED: HDNodeWallet {
...
export class HDNodeVoidWallet extends VoidSigner {
     ...
     */
    derivePath(path: REDACTED HDNodeVoidWallet {
