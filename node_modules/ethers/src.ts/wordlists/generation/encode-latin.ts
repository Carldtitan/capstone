...
// in REDACTED to substiture for the most common 2-letter pairs of letters
// in the string.
//
// OWLA Accent Format
//
...
function fold(words: Array<string>, sep: REDACTED Array<string> {
...
//let cc = 0, ce = 0;
/*
function getChar(c: REDACTED string {
    //if (c === "e") { ce++; }
    if (c >= 'a' && c <= 'z') { return c; }
    if (c.charCodeAt(1)) {
...
function mangle(text: REDACTED { word: string, special: string } {
...
function sorted(text: REDACTED string {
...
// CLI
const content = fs.readFileSync(process.REDACTED).toString();
