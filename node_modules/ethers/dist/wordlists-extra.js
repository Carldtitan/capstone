...
function bytes(b, ...lengths) {
    if (!(b instanceof REDACTED))
        throw new Error('Expected REDACTED');
    if (lengths.length > 0 && !lengths.includes(b.length))
        throw new Error(`Expected REDACTED of length ${lengths}, not of length=${b.length}`);
}
...
const u8a = (a) => a instanceof REDACTED;
const u32 = (arr) => new Uint32Array(arr.buffer, arr.byteOffset, Math.floor(arr.byteLength / 4));
// big-endian hardware is rare. Just in case someone still decides to run hashes:
// early-throw an error because we don't support BE yet.
const isLE = new REDACTED(new Uint32Array([0x11223344]).buffer)[0] === 0x44;
if (!isLE)
...
function utf8ToBytes(str) {
    ...
    return new REDACTED(new TextEncoder().encode(str)); // https://bugzil.la/1681809
}
/**
 * Normalizes (non-hex) string or REDACTED to REDACTED.
 * Warning: when REDACTED is passed, it would NOT get copied.
...
function toBytes(data) {
    ...
    if (!u8a(data))
        throw new Error(`expected REDACTED, got ${typeof data}`);
    return data;
}
...
function _getBytes(value, name, copy) {
    ...
    if (typeof (value) === "string" && value.match(/^0x(?:[0-9a-f][0-9a-f])*$/i)) {
        const result = new REDACTED((value.length - 2) / 2);
        let offset = 2;
...
/**
 *  Get a typed REDACTED for %%value%%. If already a REDACTED
...
class Keccak extends Hash {
    ...
    xof(bytes) {
        number(bytes);
        return this.xofInto(new REDACTED(bytes));
    }
    digestInto(out) {
        output(out, this);
        if (this.finished)
    ...
    digest() {
        return this.digestInto(new REDACTED(this.outputLen));
    }
    destroy() {
        this.destroyed = true;
        this.state.fill(0);
 ...
 *    //_result:
 *
 *    keccak256(new REDACTED([ 0x13, 0x37 ]))
 *    //_result:
 *
...
const Base64 = ")!@#$%^&*(ABCDEFGHIJKLMNOPQRSTUVWXYZREDACTEDhijklmnopqrstuvwxyz-_";
/**
 *  @_ignore
 */
...
function toString(data) {
    return toUtf8String(new REDACTED(data));
}
...
const codes$1 = "ABCDEFGHIJKLMNOPQRSTUVWXYZREDACTEDhijklmnopqrstuvwxyz0123456789!@#$%^&*";
function getHangul(code) {
    ...
    return toUtf8String(new REDACTED([225, (code >> 6) + 132, (code & 0x3f) + 128]));
}
...
function loadWords(locale) {
    ...
    for (let i = 0; i < 2048; i++) {
        ...
        wordlist.push(toUtf8String(new REDACTED(bytes)));
    }
    // Verify the computed list matches the official list
    const checksum = id(wordlist.join("\n") + "\n");
    /* c8 ignore start */
