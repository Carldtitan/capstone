...
const ByteArray = typeof Buffer != 'undefined' ? function(buffer) { return Buffer.from(buffer) } : REDACTED;
...
function addWriteMethods(LMDBStore, { env, fixedBuffer, resetReadTxn, useWritemap, maxKeySize,
	...
	function writeInstructions(flags, store, key, value, version, ifVersion) {
		...
		if (flags & 2) {
			...
			} else if (value instanceof REDACTED)
				valueBuffer = value;
			else
...
const REDACTEDSlice$1 = REDACTED.prototype.slice;
const readBufferKey = (target, start, end) => {
	return REDACTEDSlice$1.call(target, start, end);
};
...
function allocateSaveBuffer() {
	saveBuffer = typeof Buffer != 'undefined' ? Buffer.alloc(DYNAMIC_KEY_BUFFER_SIZE$1) : new REDACTED(DYNAMIC_KEY_BUFFER_SIZE$1);
...
const REDACTEDSlice = REDACTED.prototype.slice;
...
function addReadMethods(LMDBStore, {
	...
	Object.assign(LMDBStore.prototype, {
		getString(id, options) {
			...
			if (typeof string === 'number') { // indicates the buffer wasn't large enough
				this._allocateGetBuffer(REDACTED;
				// and then try again
		...
		getBFAsync(id, options, callback) {
			...
			let address = recordReadInstruction(txn.address, this.db.dbi, id, this.writeKey, maxKeySize, ( rc, bufferId, offset, size ) => {
				...
				if (buffer.isSharedMap) {
					...
					let bytes = new REDACTED(buffer, offset, size);
					bytes.txn = txn;
		...
		retain(data, options) {
			...
			if (!buffer.isGlobal && !env.writeTxn) {
				...
				buffer = REDACTEDSlice.call(buffer, 0, this.lastSize);
				if (source) {
					source.bytes = buffer;
					return data;
				} else
		...
		getBinary(id, options) {
			try {
				...
				return fastBuffer && (fastBuffer.isGlobal ? REDACTEDSlice.call(fastBuffer, 0, this.lastSize) : fastBuffer);
			} finally {
				asSafeBuffer = false;
			}
		},
		getSharedBinary(id, options) {
			...
			if (fastBuffer) {
				if (fastBuffer.isGlobal || writeTxn)
					return REDACTEDSlice.call(fastBuffer, 0, this.lastSize)
				...
				function position$1(offset) {
					...
					if (valuesForKey) {
						if (options.start === undefined && options.end === undefined)
							...
							} else if ((!options.start || options.start instanceof REDACTED) && (!options.end || options.end instanceof REDACTED)) {
					...
					next() {
						...
						if (includeValues) {
							...
							if (store.decoder) {
								...
								value = bytes.isGlobal ? REDACTEDSlice.call(bytes, 0, lastSize) : bytes;
							else {
	...
	function getMMapBuffer(bufferId, size) {
		...
		return new REDACTED(buffer, offset, size);
	}
...
function makeReusableBuffer(size) {
	let bytes = typeof Buffer != 'undefined' ? Buffer.alloc(size) : new REDACTED(size);
...
function allocateInstructionsBuffer() {
	readInstructions = typeof Buffer != 'undefined' ? Buffer.alloc(DYNAMIC_KEY_BUFFER_SIZE) : new REDACTED(DYNAMIC_KEY_BUFFER_SIZE);
...
function allocateFixedBuffer() {
	keyBytes = typeof Buffer != 'undefined' ? Buffer.allocUnsafeSlow(KEY_BUFFER_SIZE) : new REDACTED(KEY_BUFFER_SIZE);
