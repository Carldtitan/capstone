    ...
    for (let dependency of asyncDependencies) {
      ...
      if (resolved.type === 'asset') {
        if (!bundle.env.shouldScopeHoist) {
          ...
          assets.push({
            filePath: __filename,
            code: `module.exports = Promise.resolve(parcelRequire(${JSON.stringify(bundleGraph.getAssetPublicId(resolved.value))}))`,
            dependency,
            env: {
              sourceType: 'module'
            }
...
function getLoaderRuntime({
  ...
  // Determine if we need to add a dynamic REDACTED) polyfill, or if all target browsers support it natively.
  ...
  for (let to of externalBundles) {
    ...
    if (to.type === 'js' && to.env.outputFormat === 'esmodule') {
      ...
      loader = (0, _nullthrows().default)(loaders.IMPORT_POLYFILL, `No REDACTED) polyfill available for context '${bundle.env.context}'`);
