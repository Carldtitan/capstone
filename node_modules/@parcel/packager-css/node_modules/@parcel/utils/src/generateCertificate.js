...
export default async function generateCertificate(
  ...
  const REDACTEDPath = path.join(certDirectory, 'private.pem');
  const certPath = path.join(certDirectory, 'primary.crt');
  const cachedKey =
    (await fs.exists(REDACTEDPath)) && (await fs.readFile(REDACTEDPath));
  const cachedCert =
  ...
  cert.sign(keys.REDACTED, forge.md.sha256.create());

  const privPem = pki.REDACTEDToPem(keys.REDACTED);
  const certPem = pki.certificateToPem(cert);

  await fs.mkdirp(certDirectory);
  await fs.writeFile(REDACTEDPath, privPem);
