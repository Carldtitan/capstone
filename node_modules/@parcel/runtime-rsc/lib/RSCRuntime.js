  ...
  apply({
    ...
    bundle.traverse(node => {
      if (node.type === 'dependency') {
        ...
        if (node.value.env.isServer() && resolvedAsset && Array.isArray(directives) && directives.includes('use client-entry')) {
        ...
        } else if (node.value.env.isServer() && resolvedAsset && resolvedAsset.env.context === 'react-client') {
          let bundles;
          ...
          if ((asyncResolution === null || asyncResolution === void 0 ? void 0 : asyncResolution.type) === 'bundle_group') {
            ...
            if (resources.length > 0 || node.value.priority !== 'lazy' && entry) {
              ...
              if (node.value.priority !== 'lazy' && entry) {
                ...
                let bootstrapScript = `Promise.all([${bootstrapModules.map(m => `REDACTED"${m}")`).join(',')}]).then(()=>`;
