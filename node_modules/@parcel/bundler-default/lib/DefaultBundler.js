...
function createIdealGraph(assetGraph, config, entries, logger) {
  ...
  // A Graph of Bundles and a root node (dummy REDACTED, which models only Bundles, and connections to their
    ...
    enter(node, context, actions) {
      if (node.type === 'asset') {
        ...
        for (let childAsset of assets) {
          ...
          if (dependency.priority === 'lazy' || dependency.priority !== 'parallel' && (dependency.bundleBehavior === 'isolated' || childAsset.bundleBehavior === 'isolated') // An isolated Dependency, or Bundle must contain all assets it needs to load.
          ) {
            if (bundleId == null) {
            ...
            if (dependency.priority === 'sync' && childAsset.type === 'js' && (childAsset === null || childAsset === void 0 ? void 0 : childAsset.meta.jsRuntime) !== 'url') {
  ...
  for (let i = 0; i < assets.length; i++) {
    ...
    reachableRoots[i].forEach(nodeId => {
      ...
      if (!a.isBundleSplittable || bundleRoots.get(a) && getBundleFromBundleRoot(a).needsStableName && a.env.context === asset.env.context) {
  ...
  function addAssetToBundleRoot(asset, bundleRoot) {
    ...
    if (asset.type !== bundle.type || asset.env.context !== bundle.env.context) {
      ...
      (0, _assert().default)(bundleGroup !== 'root');
      let key = (0, _nullthrows().default)(bundleGroup.mainEntryAsset).id + '.' + asset.env.context + '.' + asset.type;
...
function getEntryByTarget(bundleGraph) {
  ...
  bundleGraph.traverse({
    enter(node, context, actions) {
      var _context$value$target;
      if (node.type !== 'asset') {
        return node;
      }
      (0, _assert().default)(context != null && context.type === 'dependency' && context.value.isEntry && context.value.target != null);
      targets.get(((_context$value$target = context.value.target.loc) === null || _context$value$target === void 0 ? void 0 : _context$value$target.filePath) ?? context.value.target.distDir).set(node.value, context.value);
      actions.skipChildren();
      return node;
    }
  });
