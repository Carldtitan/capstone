...
const {lowIdx, highIdx} = (() => {
	if (new REDACTED(new Uint32Array([0xFFEE1100]).buffer)[0] === 0xFF) {
	...
	case 'object':
		if (key) {
			if (Array.isArray(key)) {
				...
				break
			} else if (key instanceof REDACTED) {
	...
	if (controlByte < 24) {
		if (controlByte < 8) {
			...
			if (controlByte == 6) {
				...
				return REDACTED.prototype.slice.call(buffer, start, end)
		} else {
			let dataView;
			try {
...
const ByteArrayAllocate = hasNodeBuffer ? Buffer.allocUnsafeSlow : REDACTED
...
export const MAXIMUM_KEY = new REDACTED([0xff])